...
ov_model = YOLO("/path/to/your/test6_openvino_model")
...
def detect_green(frame):
    ...
    lower_green = np.array([40, 40, 40])  # Adjusted lower bound
...
def detect_blue(frame):
    hsv = preprocess_frame(frame)
    lower_blue = np.array([79, 31, 152])
    upper_blue = np.array([99, 131, 252])
...
def generate_frame():
    ...
    while True:
        ...
        # Calculate quadrant safe points based on the field dimensions
        if conversion_factor_set and field_rectangle is not None:
            (x, y, w, h) = cv2.boundingRect(field_rectangle)
            cx, cy = get_center([x, y, x + w, y + h])
            quadrant_safe_points[1] = (cx - w // 4, cy - h // 4)
            quadrant_safe_points[2] = (cx + w // 4, cy - h // 4)
            quadrant_safe_points[3] = (cx + w // 4, cy + h // 4)
            quadrant_safe_points[4] = (cx - w // 4, cy + h // 4)
